...
def dump_jsonlines(data: Iterable[Any],
                   path: Union[str, Path],
                   **json_kwargs):
    """
    Write each item from `data` to `path` as one JSON object per line.

    Example:
        dump_jsonlines(results, "dolly15k_generations.jsonl", ensure_ascii=False)
    """
    path = Path(path)
    ...
    with path.open("w", encoding="utf-8") as f:
        for obj in data:
            f.write(json.dumps(obj, **json_kwargs))
            f.write("\n")